...
subroutine init_grid(mype,npes)
  ...
  temp = (xposition - 0.5*(nxslice-1)) * nx
  x = 0.5*(xmax + xmin) + dx*(temp + (/ (i-1,i=iu1,iu2) /) - 0.5*(nx-1))
#else
  !x=xmin +                   dx/2 + (/ (i-1,i=iu1,iu2) /)*dx
  x = 0.5*(xma x +xmin) + dx*(       (/ (i-1,i=iu1,iu2) /) - 0.5*(nx-1))
#endif
#if NDIM > 1
  if (mype==0) write(*,*) '  dy',dy
#if WITHMPI==1
  !y=ymin + dy*ny*yposition + dy/2 + (/ (j-1,j=ju1,ju2) /)*dy
  temp = (yposition - 0.5*(nyslice-1)) * ny
  y = 0.5*(ymax + ymin) + dy*(temp + (/ (j-1,j=ju1,ju2) /) - 0.5*(ny-1))
#else
  !y=ymin +                   dy/2 + (/ (j-1,j=ju1,ju2) /)*dy
  y = 0.5*(ymax + ymin) + dy*(       (/ (j-1,j=ju1,ju2) /) - 0.5*(ny-1))
#endif
#endif
#if NDIM==3
  ...
  temp = (zposition - 0.5*(nzslice-1)) * nz
  z = 0.5*(zmax + zmin) + dz*(temp + (/ (k-1,k=ku1,ku2) /) - 0.5*(nz-1))
#else
  !z=zmin +                   dz/2 + (/ (k-1,k=ku1,ku2) /)*dz
  z = 0.5*(zmax + zmin) + dz*(       (/ (k-1,k=ku1,ku2) /) - 0.5*(nz-1))
#endif
#endif
