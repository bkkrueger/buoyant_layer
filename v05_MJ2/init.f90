...
#if WITHMPI==1
  call mpi_param
  if (modulo(nx_glob,nxslice) /= 0) then
     write (*,*) "nx_glob must be an integer multiple of nxslice"
     stop
  end if
  if (modulo(ny_glob,nyslice) /= 0) then
     write (*,*) "ny_glob must be an integer multiple of nyslice"
     stop
  end if
  if (modulo(nz_glob,nzslice) /= 0) then
     write (*,*) "nz_glob must be an integer multiple of nzslice"
     stop
  end if
  nx = nx_glob/nxslice
  ny = ny_glob/nyslice
