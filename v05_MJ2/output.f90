...
subroutine output(uin,x,y,z,time,dt,dx,dy,dz,nhist,ndump,nspec,mype,npes)
  ...
  integer :: ndump,nhist,nspec,mype,npes,i
  ...
  if(use_collective_io) then
#if PARAHDF5==1
     ! First, write small metadata with proc #0, then open the file collectively
     if(mype==0)then
        call H5Fcreate_f(filename, H5F_ACC_TRUNC_F, file_id, ierr)

        !Dataset "para_real"; Type real; dim=1; size=nb_real
        para_real=(/ time,dt,dx,dy,dz, x_mass_old, x_advc_old, time_old, &
        ...
        !Dataset "boxSize"; Type real; dim=1; size=6
  ...
  if (.not. use_collective_io) then
     call dump_4d_array_h5(file_id, "ss", ss0, n1, SS_NVAR, 1, 1)
  endif
  ! - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  if(use_collective_io) then
     ...
     ! Only a single line will perform this communication
     if (yposition == 0 .and. zposition == 0) then
        ...
        call H5Fopen_f(filename, H5F_ACC_RDWR_F, file_id, ierr, &
                       access_prp=fapl_id)
        ! Write the data
        call dump_1d_array_h5_special(file_id, "ss_dens", ss0(:,1), n1, mype)
        call dump_1d_array_h5_special(file_id, "ss_momx", ss0(:,2), n1, mype)
        call dump_1d_array_h5_special(file_id, "ss_ener", ss0(:,3), n1, mype)
        ! Close the file
        call H5Fclose_f(file_id, ierr)
...
subroutine output(uin,x,y,z,time,dt,dx,dy,dz,nhist,ndump,nspec,mype,npes)
  ...
  integer :: commx
  integer :: sxdimid, svdimid
  integer, dimension(2) :: ssdimid
  integer :: ssid
  integer(kind=MPI_OFFSET_KIND), dimension(2) :: dimss, startss, countss
  integer(kind=MPI_OFFSET_KIND) :: nvar_mpi
  ...
  if(mype==0) then
     ...
     ! Define dimensions
     nbint = nb_int; nbreal = nb_real; nbmpi = nb_mpi
     ...
     nout = nfmpi_def_var(ndid, "para_mpi", NF_INT, 1, (/nmid/), pmid)
...
#if WITHMPI==1
     nout = nfmpi_put_vara_int_all(ndid, pmid, (/1_8/), (/nbmpi/), para_mpi)
#endif
  ...
  nout = nfmpi_put_vara_double_all(ncid, Bxrid, start, count, uin(1,:,:,:,9))
  nout = nfmpi_put_vara_double_all(ncid, Byrid, start, count, uin(1,:,:,:,10))
  nout = nfmpi_put_vara_double_all(ncid, Bzrid, start, count, uin(1,:,:,:,11))

  ! Close file
  nout = nfmpi_close(ncid)

  ! Reopen the file in order to write the steady-state using only a single
  ! slice along the x direction
  nvar_mpi = SS_NVAR
  if (mype == 0) write(*,*) "Creating communicator"
  call create_comm_line(commx, 1, mype)
  if (yposition == 0 .and. zposition == 0) then
     ! Open the file only using the (x,0,0) communicator
     if (mype == 0) write(*,*) "Opening file"
     nout = nfmpi_open(commx, filename, ior(NF_WRITE, NF_64BIT_OFFSET) &
          & ,MPI_INFO_NULL, ncid)
     ! Define steady state dimensions
     if (mype == 0) write(*,*) "Defining dimensions"
     nout = nfmpi_def_dim(ncid, "nsx", nxglob, sxdimid)
     nout = nfmpi_def_dim(ncid, "nsv", nvar_mpi, svdimid)
     ssdimid = (/ sxdimid, svdimid /)
     ! Create steady state variable
     if (mype == 0) write(*,*) "Creating variable"
     nout = nfmpi_def_var(ncid, "ss0", NF_DOUBLE, 2, ssdimid, ssid)
     ! End definitions
     if (mype == 0) write(*,*) "Ending definitions"
     !nout = nfmpi_enddef(ncid)
     ! Position of steady-state
     if (mype == 0) write(*,*) "Computing positioning"
     dimss = (/ nxglob, nvar_mpi/)
     startss = (/ xposition, 0 /) * dimss + 1
     countss = dimss
     ! Write steady state
     if (mype == 0) write(*,*) "Writing data"
     nout = nfmpi_put_vara_double_all(ncid, ssid, startss, countss, ss0(:,:))
     ! Close the file
     if (mype == 0) write(*,*) "Closing file"
     nout = nfmpi_close(ncid)
  end if
  if (mype == 0) write(*,*) "Barrier"
  call MPI_Barrier(commx, ierr)
  if (mype == 0) write(*,*) "Freeing communicator"
  call MPI_Comm_Free(commx, ierr)
