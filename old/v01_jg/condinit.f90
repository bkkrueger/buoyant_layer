...
SUBROUTINE condinit(mype)
  ...
  ! Compute the amplitude and phase (random) of the Fourrier components of the perturbations
  IF (pert .EQ. 'rho') THEN
     iseed=pertseed
  ...
  y0(2) = 1.d0  ! c
  DO i=iu2,iu1,-1
     ! compute the stationary flow at x = x(i) :
     IF (i .EQ. iu2) THEN
        x0 = xmax
        dx0 = (x(0)-x(1))/nrk
     ENDIF
	...
	CALL rkutta0(x0, y0, dx0, ncount)
     ...
     ! Intialise the variable uin with the stationary flow + perturbations 
     DO l=1,ngrid
        DO k=ku1,ku2
           DO j=ju1,ju2
              ...
              ENDIF
              IF (pert .EQ. 'rho') THEN
                 ! add density perturbations in the heating layer with the fourrier modes computed before
                 IF ((x(i) .GT. -1.d0).AND.(x(i) .LT. 1.d0)) THEN
                    DO irand = nxmin,nxmax
                       DO jrand = nymin,nymax
