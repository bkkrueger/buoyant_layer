      ...
      if (pert .eq. 'rhogrid') then ! -----------------------------------------
         ...
         do irand = nxmin, nxmax
            do jrand = nymin, nymax
               ...
               ! layer, so instead of the domain limits you instead have an
               ! effective xmin (xmax) of -1 (+1).
               theta1 = irand*(2.d0*PI*x(i)/2.d0 + &
                        fourier_phasex(irand,jrand))
               theta1 = jrand*(2.d0*PI*y(j)/(ymax-ymin) + &
...
subroutine condinit(mype)
   ...
   namelist /physics_params/ Mup, shape_width, Kheat, Kgrav, gamma, &
...
double precision function layer_shape(x0)
   use heating_layer
   implicit none
   double precision :: x0
   ...
   !   layer_shape = 1.0d0
   !end if
   layer_shape = exp(-(x0/shape_width)**2)

   return
end function layer_shape
...
subroutine give_gravity(x0, gravity)
    ...
    double precision :: layer_shape

    gravity = -1.0d0 * Kgrav * layer_shape(x0)
...
subroutine give_cool(x0,rho,P,cool)
   ...
   cool = (Kheat * rho * Mup / gamma) * layer_shape(x0)
