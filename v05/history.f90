subroutine history(uin,x,time,dt,dx,dy,dz,mype,npes)
   ...
   ! TODO: Add to output and restart_run: ss0, all 5 of the *_old variables
   use perturb, only : pert_type, bubble_x, x_geom_old, x_advc_old,           &
                       dv_grav_old, dv_buoy_old, dv_drag_old
   use steady_state, only : ss0, SS_DENS
...
#  endif
   ...
   real(dp), parameter :: BUBBLE_CUTOFF = 1.0d-3
...
#  if WITHMPI==1
   ...
   call MPI_Allreduce(sendbuf, recvbuf, 1, MPI_DOUBLE_PRECISION, MPI_MAX,     &
                      MPI_COMM_WORLD, ierr)
...
#  endif
   ...
   if (time == 0.0d0) then
      ...
      v_bubble = (x_geom - x_geom_old) / dt
   end if

   ! Compute advection position
   if (time == 0.0d0) then
      x_advc = bubble_x + v_advc(bubble_x,x,dx) * dt
   else
      x_advc = x_advc_old + v_advc(x_advc_old,x,dx) * dt
   end if
      ...
      do j = jlo, jhi
         do i = ilo, ihi
            ...
            if (bubble(i,j,k) > BUBBLE_CUTOFF * bubble_max) then
               ...
               sum_rho0_g_dV = sum_dens_g_dV + cell_vol * rho0 * grav
            end if
         end do
      end do
   end do
...
#  endif
   ...
   dv_grav     = dv_grav_old + a_grav * dt
   dv_buoy     = dv_buoy_old + a_buoy * dt
   dv_drag     = dv_drag_old + a_drag * dt
   dv_grav_old = dv_grav
   dv_buoy_old = dv_buoy
   dv_drag_old = dv_drag
   ...
   if (mype == 0) then
      if (time == 0.d0) then
         ...
         write(LUN,"('#',x,a13,50(3x,a13))") "time", "bubble_max",            &
            "X-section/volume",                                               &
            "x_advc", "x_geom", "x_mass", "x_bubble", "v_bgnd",               &
